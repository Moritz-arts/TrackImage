...
/* The open picture carries its own copy of the folder tree, so the same branch
   exists twice under the same id -- getElementById only ever reached the first
   one, which in full view is the hidden gallery copy, and a branch clicked
   beside the picture stayed shut. Every copy is folded, which also keeps the
   two in step. */
function toggleFolderIdx(idx){var key=_folderPaths[idx];S.openFolders[key]=!S.openFolders[key];if(!S.openFolders[key])delete S.openFolders[key];_lsSave('ti_open_folders',S.openFolders);var open=!!S.openFolders[key];document.querySelectorAll('.tree-children[data-fidx="'+idx+'"],.arrow[data-fidx="'+idx+'"]').forEach(function(el){el.classList.toggle('open',open);});}
