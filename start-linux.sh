#!/bin/bash
# v4.33: everything TrackImage owns lives one level down, in Trackimage_files.
# This launcher is the only thing in the unpacked folder, so it steps into that
# directory itself before doing anything else.
TIDIR="$(cd "$(dirname "$0")" && pwd)/Trackimage_files"
...
echo ""
echo "  ========================================"
echo "   TrackImage v4.61 - Setup & Start"
echo "  ========================================"
echo ""
...
if ! python3 -c "import tkinter" &> /dev/null 2>&1; then
    echo "  [note] python3-tk is not installed. Everything works without it;"
    echo "         you will type a folder path instead of picking it in a"
    echo "         dialog. To get the dialog: sudo apt install python3-tk"
fi
...
# v3.76: keep the venv self-contained - a per-user pip install of the same
# nvidia wheels must never leak in (mixing cuDNN versions breaks GPU tagging).
...
echo ""
# v4.06: TrackImage opens its own window. pywebview needs the WebKitGTK bindings
# from the distribution -- they cannot come from pip. Check once and say so
# plainly instead of silently ending up in a browser.
if ! python3 -c "import gi" >/dev/null 2>&1; then
  echo "  ! The window mode needs the GTK bindings of your distribution:"
  ...
  echo "    Without them TrackImage opens in your browser instead."
  echo ""
fi
# Detached, with the console output going to the logfile -- the terminal is free
# again immediately and the app keeps running when it is closed.
...
RC=$?
if [ $RC -eq 0 ]; then
    echo "  TrackImage is running - http://$URLHOST:5001"
    echo "  Output goes to Trackimage_files/Userdata/Logs/trackimage.log and to Settings > Console."
elif [ $RC -eq 2 ]; then
